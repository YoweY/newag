...
/**
 * 作者：jksfood on 2017/3/28 09:20
 * 录音时的动画效果
 */

public class VoiceRecording extends View{
    ...
    public VoiceRecording(Context context) {
        this(context,null);
    }

    public VoiceRecording(Context context, AttributeSet attrs) {
        super(context, attrs);
...
//        canvas.drawText(unit == null ?"s":unit,timeWidth, getHeight()/2-20, paint1);
    ...
    private void drawVoiceLine(Canvas canvas) {
        ...
        canvas.save();
        int moveY = getHeight()*2/4;
        for (int i = 0; i < paths.size(); i++) {
            paths.get(i).reset();
            paths.get(i).moveTo(getWidth()*5/6, getHeight() *2/4);
        }
